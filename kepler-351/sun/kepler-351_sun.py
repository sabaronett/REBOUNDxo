...
T0 = 12383.5e6          # Sun's age ~ 10 Myr pre-TRGB (sim start)
M0 = 0.9370753865330389 # initial mass of star
names = ['a', 'b', 'c', 'd']
symbols = ['a', 'b', 'c', 'd']
...
def makesim():
    ...
    sim.add(m=3.e-6*3.06**3, a=0.214, hash=names[1])
    sim.add(m=3.e-6*3.19**3, a=0.287, hash=names[2])
    sim.add(m=3.e-6*2.72**3, a=0.534, hash=names[3])
...
for k in range(Nloops):
    ...
    ts = np.linspace(0., 5e6, Nout)    # total sim integration time
    ...
    with IncrementalBar('Integrating...', max=Nout,
        ...
        for i, t in enumerate(ts):
            ...
            if r <= ps[0].r or ps[cp].a < 0: # nearest planet engulfed
                ...
                if cp < sim.N:
                    sim.dt = 0.05*ps[cp].P   # adjust timestep accordingly
                else:                        # all planets engulfed
                    break
